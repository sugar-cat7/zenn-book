...
この記事は「ひとりで気になるOSSのソースコード全部読んで何かする Advent Calendar 2025」2日目の記事です。
https://qiita.com/advent-calendar/2025/sugarcat

- [全体編](https://zenn.dev/king/articles/4e55f030aa01ee)
- [GameServer編](https://zenn.dev/king/articles/003f7b79409b4a) ←今ここ
- Fleet編
...
### GameServerに関して
AgonesではCustom ResourceとしてGameServerを提供しています。
...
この記事ではGameServer CRDに焦点を当てて内部実装を眺めていきます。
GameServerの定義を眺めてみるとユーザー側で行える特徴的な設定はコンテナに対するPort Allocationの設定やプロトコルの指定、GameServer自体に対してRoom数やキャパシティの設定ができる点などが普通のDeployment等とは異なります。
...
Kubernetes APIに対して送信されているGameServerの定義は以下で確認できます。
...
GameServerは内部でStateを持っており、Pod起動時からShutdonwnまでのライフサイクルを管理しています。
...
状態遷移はざっくり下記が行われ、GameServerのStateが遷移していきます。
- Specで指定されたDynamic PolicyによるPort Allocation
- Pod作成
- SDKによるReady/Shutdownシグナルの受信

![GameServer States](https://agones.dev/site/diagrams/gameserver-states.dot.png)

まずControllerのRunメソッドではWorkQueueに対して登録された`syncGameServer`が呼び出されており、その中のState遷移の主要な部分が実装されています。
- Kubernetes APIサーバーからのイベント（Add/Update/Delete）を監視
- 状態変化があった場合のみWorkQueueにエンキューし、Reconciliation Loopを実行
...
それぞれのStateに応じてEnqueueされ処理が行われます。
...
まず`syncGameServerPortAllocationState`の内部でPort Allocationの処理が行われています。
...
この`Allocate`メソッドでDynamic Policyに基づいてPortの割り当てが行われています。
```go
gsCopy := c.portAllocator.Allocate(gs.DeepCopy())
```

Allocaterは各ノードのPortを調べ必要Port及びポリシーが満たせるPortを探し、見つかれば`gameServerRegistry`へ登録しています。
割り当てたGameServerはキーはUIDで管理されています。
```go
UID types.UID `json:"uid,omitempty" protobuf:"bytes,5,opt,name=uid,casttype=k8s.io/kubernetes/pkg/types.UID"`
```
複数のGameServerが同じノードで同じPortを使用しないように、`mutex.Lock()`が使用されスレッドセーフな実装になっています。
...
またポート不足時に仮想的にPortを割り当てておき、Podの状態をPendingにすることで再スケジューリングを促す仕組みも実装されています。
...
このプロセスが完了するとPod作成(`syncGameServerCreatingState`)が行われます。
内部で`agones-gameserver-sidecar`のコンテナが注入されます。
これはGameServerプロセスがAgonesと通信するためのAPIサーバーとして動作します。
gRPCとHTTPの両方で通信が可能でGameServer側のSDK経由で利用され、内部通信向けのProxyとして動作しています。
...
またユーザー定義のコンテナもPodSpecに追加され、Podが作成されます。
https://github.com/googleforgames/agones/blob/67c4b6b39b9f4a0b6c473e303acb38c2cccf150e/pkg/gameservers/controller.go#L664

それぞれのコンテナに対して適切な権限を持つServiceAccountが割り当てられるようにGameServer側のトークン(`/var/run/secrets/kubernetes.io/serviceaccount`)を空のVolumeでオーバーライドしています。
...
その後Pod起動に合わせて状態が遷移していきます。SDK <-> Sidecar経由でKubernetes APIサーバーへのリクエストが行われていきます。
...
操作自体にはCode Generatorで作成されたClientSetが使用されています。
...
Podが削除されたのにGameServerが残っている場合にUnhealthyに移行
...
- `succeededController`
PodがSucceeded状態になった場合、GameServerをShutdown状態に移行（Sidecar Containers feature用）
...
GameServerのライフサイクルについてみていきました。
明日はFleet等のGameServer群を管理するリソースについてみます。