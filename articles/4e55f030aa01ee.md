...
カレンダーの通り、個人的に気になっていることを調べて満足するだけの自己満足記事です。
大体↓の流れで進みます。Kubernetesや各種クラウド等の知識はある前提で書いています。初回以外は基本内部実装を追っていきます。特に人に見せることを目的にしていないためかなり雑です。

- 全体編 ←今ここ
- GameServer CRD編
- GameServer Controller編
- Admission Webhooks編
- Fleet CRD編
- Fleet Controller編
- GameServerAllocation CRD編
- Allocation Controller/APIService編
- FleetAutoscaler CRD編
- FleetAutoscaler Controller編
- Allocator Service編
- Agones Coreコマンド編
- Metrics編
- 各種Cloud統合編
...
## 全体概要編
## Agonesに関して
Dedicated GameServerを構成するコンポーネント群をKubernetes上で扱えるようにしたCustom Resource群です。
...
Agonesの誕生
下記の通り、マッチメイキングの仕組み(Open Match等)と組み合わせて使うことを想定されており、Dedicated Game Server Hostingのための仕組みとして設計されています。
...
Prometheusとの統合も可能で、GameServerやFleetのメトリクスを収集できるようになっています。
https://agones.dev/site/docs/guides/metrics/

Grafanaダッシュボード等のConfigMapも提供されておりAgonesで監視する対象を簡単に可視化できます。
...
### ネットワーク
UDPでの接続が基本となるので、ワーカーノードに対して接続するため7000~8000のポートを外部ファイアウォールで常時開放する必要があります。
KubernetesのHostPort機能を利用し、動的にノードのIPとPortの組み合わせを使用してクライアントからの接続を可能にしています。
https://agones.dev/site/docs/reference/gameserver/

### ユースケース
基本的にOpenMatchと組み合わせたマルチプレイ系のゲームでの利用が多いです。
https://github.com/googleforgames/open-match
ただそれ以外にもWebSocketのサーバーのホスティング先としての利用や音声、AI系の話題もありました。
以下リンクだけ貼っておきます。
