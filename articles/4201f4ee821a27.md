...
※ メッセージに対しては最大 8MB までのファイルを添付することができます。
...
リッチなメッセージ形式で、より多くの情報を整然とした形で表示できます。
...
メッセージ内でのインタラクションを提供するコンポーネントです。
...
この時間制限を考慮し、重い処理を行う場合は非同期で行う必要があります。
...
| JavaScript | Node.js 上で動作する Bot 向けのライブラリ。非同期処理をサポートし、効率的な Bot 開発ができる。 | [discord.js](https://discord.js.org/#/), [Eris](https://abal.moe/Eris/) |
| Java | 型安全性を活かした Bot 開発が可能。大規模な Bot の開発・運用に適している。 | [JDA](https://github.com/DV8FromTheWorld/JDA), [Discord4J](https://discord4j.com/) |
...
| Rust | 高性能かつ安全性を重視した Bot の開発が行える。 | [Serenity](https://github.com/serenity-rs/serenity) |
...
Discord Game SDK と他のゲームプラットフォーム（例: Valve の Steamworks SDK）のロビーとネットワーク層は類似しています。以下の OSS を使用することで、共有機能のための統一されたインターフェースを開発者に提供し、複数のプラットフォームでの開発を簡略化することができます。
...
インテントは、特定のイベント群を Bot にサブスクライブさせるためのものです。
...
WebSocket で常時チャンネル内のイベントを Listen するために常時起動するためのマネージドなサーバーを提供する代表的なサービスは以下のとおりです。
...
常駐の必要がなく HTTP Interaction のみの利用で十分な場合は、Lambda や Cloudflare Workers などの Faas によるホスティングも可能なため検討して見てください。
...
- 公開されている DiscordBot の一覧がランキング形式で閲覧できます。
