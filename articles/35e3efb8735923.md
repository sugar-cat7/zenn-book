...
Webアプリケーションをトレースする場合、理想的には「ルーターがリクエストを処理したこと」がトレースのルートスパンとなるべきです。しかしOrchestionではコールグラフを辿り計装されるため一般的なRouterLibrary(net/httpのラッパー)では`net/http`パッケージも自動的に計装されるため、Root Spanが`net/http`のスパンとなります。

```
net/http.Server -> chi.Router -> handler(own span)
```

この結果、実際のトレース構造では、ルーターのスパンではなく`net/http`のスパンがルートスパンとなります。
...
そのためErrorTrackingには連携されるけれども、エラーの内容は伝搬されていないスパンが誕生してしまいます。
...
`DD_SERVICE`を指定しない場合、バイナリ名がそのままサービス名として使用されます。これを利用し、各サービス名をバイナリ名で表現し、baseService+Service OverrideされたRouterのスパン名でErrorTrackingを行うことで、エラーの内容を正確に伝播させることができます。
